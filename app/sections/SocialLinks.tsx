'use client';

import { useHoverAnimation } from '../hooks/useHoverAnimation';
import { ANIMATION_CONFIG } from '../styles/shared';
import { SiGithub, SiLinkedin, SiGmail, SiHackerrank } from 'react-icons/si';

interface SocialLink {
  platform: string;
  url: string;
  label: string;
}

interface SocialLinksProps {
  links: SocialLink[];
  className?: string;
}

// Simple SVG icons for each platform
const getIcon = (platform: string) => {
  const icons = {
    GitHub: <SiGithub className="size-6" />,
    LinkedIn: <SiLinkedin className="size-6" />,
    Twitter: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="size-6">
        <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/>
      </svg>
    ),
    Email: <SiGmail className="size-6" />,
    HackerRank: <SiHackerrank className="size-6" />
  };
  
  return icons[platform as keyof typeof icons] || null;
};

function SocialLinkItem({ link }: { link: SocialLink }) {
  // Use shared hover animation hook (DRY principle)
  const { ref, handleMouseEnter, handleMouseLeave } = useHoverAnimation<HTMLAnchorElement>({
    scale: [1, 1.1],
    translateY: [0, -4],
    duration: ANIMATION_CONFIG.durations.fast,
  });

  return (
    <li>
      <a
        ref={ref}
        href={link.url}
        target="_blank"
        rel="noopener noreferrer"
        className="group flex items-center justify-center text-white/70 hover:text-white transition-colors duration-300 relative"
        aria-label={`Visit ${link.platform} profile`}
        title={link.platform}
        onMouseEnter={handleMouseEnter}
        onMouseLeave={handleMouseLeave}
      >
        <span className="self-center">
          {getIcon(link.platform)}
        </span>
        {/* Tooltip */}
        <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-black/90 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-10">
          {link.platform}
        </span>
      </a>
    </li>
  );
}

export default function SocialLinks({ 
  links, 
  className = '' 
}: SocialLinksProps) {
  return (
    <nav 
      className={`social-links ${className}`}
      aria-label="Social media links"
    >
      <ul className="flex gap-6">
        {links.map((link) => (
          <SocialLinkItem key={link.platform} link={link} />
        ))}
      </ul>
    </nav>
  );
}
